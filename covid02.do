...
**CS model
**d(h)
**predicted cases/deaths smaller than with d(e)

log using covid02.log, replace
...
forval i=1/3 {
	...
	if (`i'==3) {
		...
		graph export "C:\Users\03638881\Dropbox\merror\covid\text\gr0.png", replace
		preserve
		...
		forval s=1/7 {
			...
			noi tsline zz if r==`s', ms(i) lc(gs0) lp(solid) || tsline yy if r==`s', ms(i) lc(gs6) lp(dash) ///
				...
				legend(col(1) size(small) rowg(*.5) keyg(*.5) symysize(*.5) symxsize(*.8))
			graph export "C:\Users\03638881\Dropbox\merror\covid\text\gr`s'.png", replace
		}
		restore
	}
...
forval i=3/5 {
	...
	if (`i'==5) {
		...
		noi tsline zz if iso=="USA", ms(i) lc(gs0) lp(solid) || tsline yy if iso=="USA", ms(i) lc(gs6) lp(dash) ///
			...
			legend(col(1) size(small) rowg(*.5) keyg(*.5) symysize(*.5) symxsize(*.8))
		graph export "C:\Users\03638881\Dropbox\merror\covid\text\grd0.png", replace
		preserve
		...
		forval s=1/7 {
			...
			noi tsline zz if r==`s', ms(i) lc(gs0) lp(solid) || tsline yy if r==`s', ms(i) lc(gs6) lp(dash) ///
				...
				legend(col(1) size(small) rowg(*.5) keyg(*.5) symysize(*.5) symxsize(*.8))
			graph export "C:\Users\03638881\Dropbox\merror\covid\text\grd`s'.png", replace
		}
		restore
		preserve 
		use xtdeaths, clear
		...
		gr export "C:\Users\03638881\Dropbox\merror\covid\text\grem.png", replace
		...
		gr export "C:\Users\03638881\Dropbox\merror\covid\text\gremc.png", replace
		restore
	}
	drop s ss z zz y yy u
...
esttab m1 m2 m3 d3 d4 d5 ///
	using "C:\Users\03638881\Dropbox\merror\covid\text\cs_h.tex", ///
