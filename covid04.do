...
glo xsave "${xs} ${txs} ${xs2}"
glo xs "${xs} ${xs2} ${txs} rt i.reg"
...
forval i=1/3 {
	...
	xtfrontier `y' $xs index`i', ti diff constraint(1)
	eststo m`i'
	predict u, u
	g z=`y' if e(sample)
	g y=z+u
	...
	if (`i'==2) {
		...
		graph export "C:\Users\03638881\Dropbox\merror\covid\text\grxt0.png", replace
		preserve
		...
		forval s=1/7 {
			...
			noi tsline zz if r==`s', ms(i) lc(gs0) lp(solid) || tsline yy if r==`s', ms(i) lc(gs6) lp(dash) ///
				...
				legend(col(1) size(small) rowg(*.5) keyg(*.5) symysize(*.5) symxsize(*.8))
			graph export "C:\Users\03638881\Dropbox\merror\covid\text\grxt`s'.png", replace
		}
		restore
	}
...
forval i=3/5 {
	...
	xtfrontier `y' $xs index`i' `sample', ti diff constraint(1)
	eststo d`i'
	predict u, u
	g z=`y' if e(sample)
	g y=z+u
	...
	if (`i'==4) {
		...
		noi tsline zz if iso=="USA", ms(i) lc(gs0) lp(solid) || tsline yy if iso=="USA", ms(i) lc(gs6) lp(dash) ///
			...
			legend(col(1) size(small) rowg(*.5) keyg(*.5) symysize(*.5) symxsize(*.8))
		graph export "C:\Users\03638881\Dropbox\merror\covid\text\grdxt0.png", replace
		preserve
		keep id iso country zz yy region tt2
		...
		forval s=1/7 {
			...
			noi tsline zz if r==`s', ms(i) lc(gs0) lp(solid) || tsline yy if r==`s', ms(i) lc(gs6) lp(dash) ///
				...
				legend(col(1) size(small) rowg(*.5) keyg(*.5) symysize(*.5) symxsize(*.8))
			graph export "C:\Users\03638881\Dropbox\merror\covid\text\grdxt`s'.png", replace
		}
		restore
		preserve 
		use xtdeaths, clear
		...
		gr export "C:\Users\03638881\Dropbox\merror\covid\text\grxtem.png", replace
		...
		gr export "C:\Users\03638881\Dropbox\merror\covid\text\grxtemc.png", replace
		restore
	}
	drop s ss z zz y yy u
...
esttab m1 m2 m3 d3 d4 d5 ///
	using "C:\Users\03638881\Dropbox\merror\covid\text\xt1.tex", ///
	...
	starlevels(* 0.10 ** 0.05 *** 0.01) label mlabels("" "" "" "" "" "" "" "" "" "" ) ///
	 ...
	 Linear time trend and region fixed effects included in all models. * p $<$.10, ** p$<$ .05, *** p$<$.01.} \end{tablenotes} )
*/
